...
def summary_full_desc_atomic_task_eval(eval_data):
    all_commands = []
    durations = []
    non_error_durations = []
    errors = []
    ...
    for scenario in eval_data:
        ...
        for task in scenario['content']['list']:
            ...
            if 'command' in task and re.match(r'^(kubectl )|(git )|(docker )', task['command']):
    ...
    return {
        ...
        'non_az_commands': [cmd['command'] for cmd in non_az],
